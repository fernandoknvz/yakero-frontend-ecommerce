...
export function ProductCard({ product, onSelect }: ProductCardProps) {
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={`flex w-full items-center gap-3 rounded-3xl border border-gray-100 bg-white p-3 text-left shadow-sm transition-all active:scale-[0.98] ${
        !product.is_available ? 'cursor-not-allowed opacity-50' : 'hover:shadow-md'
      }`}
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex items-center justify-between gap-3">
          ...
          {hasModifiers ? <span className="text-xs text-gray-400">Personalizable</span> : null}
        </div>
      </div>
